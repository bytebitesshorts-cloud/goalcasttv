  ...
  try {
    ...
    if (data.active) {
      ...
      if (res.ok) {
        ...
        let eventsToday = events.filter(event => {
          const eventDate = new Date(event.date);
          return eventDate >= today && eventDate < new Date(today.getTime() + 24 * 60 * 60 * 1000);
        });

        // Filter by allowed leagues if configured
        const allowedLeagues = data.leagues || [];
        if (allowedLeagues.length > 0) {
          const lowerLeagues = allowedLeagues.map((l: string) => l.toLowerCase().trim());
          eventsToday = eventsToday.filter((event: ESPNEvent) => {
            const compName = event.competitions?.[0]?.name?.toLowerCase() || '';
            return lowerLeagues.some((l: string) => compName.includes(l));
          });
        }
    ...
    return NextResponse.json({ active: data.active, leagues: data.leagues || [], matches: [] });
  } catch {
...
export async function PUT(req: NextRequest) {
  ...
  try {
    ...
    const updated = {
      ...
      leagues: body.leagues !== undefined ? body.leagues : (current.leagues || []),
    };
...
async function getTicker() {
  ...
  if (!tickerStore) {
    return { active: false, useLiveFeed: true, leagues: [], matches: [] };
  }
